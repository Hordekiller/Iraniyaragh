        ...
        <div className="flex items-center gap-4 lg:gap-8 h-[64px] lg:h-[76px]">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-600 font-medium tracking-widest">ARAD TOOLS • از ۱۳۸۵</div>
            </div>
          ...
          <nav className="hidden lg:flex items-center gap-7 mr-6 text-[14px] font-medium text-slate-700">
            <a href="#home" className="text-[#C2410C] font-bold flex items-center gap-1">خانه <span className="w-1.5 h-1.5 rounded-full bg-[#C2410C]" /></a>
            ...
            <a href="#blog" className="hover:text-[#FF4D00] transition flex items-center gap-1">مجله آموزشی <span className="px-1.5 py-0.5 rounded bg-emerald-700 text-white text-[10px] font-bold">جدید</span></a>
          ...
          <div className="flex items-center gap-2 mr-auto lg:mr-0">
            ...
            <button aria-label="باز کردن منو" className="lg:hidden w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center" onClick={() => show('منو')}>
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="lg:hidden overflow-hidden">
              <div className="pb-4">
                <div className="relative">
                  <input id="site-search-mobile" autoFocus value={searchQuery} onChange={e => onSearchChange(e.target.value)} aria-label="جستجو در محصولات" placeholder="جستجوی ابزار..." className="w-full h-12 pr-11 pl-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:outline-none focus:border-[#FF4D00]" />
                  <Search size={18} aria-hidden="true" className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
                </div>
              </div>
            </motion.div>
