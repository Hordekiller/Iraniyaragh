...
| `0.3` Inventory       | Foundation + protected HTTP | Transactional ledger/reservation service; protected balance/adjustment API merged (#217); full inventory HTTP contracts (warehouse/location CRUD, HTTP reservations, transfer state machine) in #215 | Expiry worker, admin UI and remaining EXEC_G4 follow-ups |
