...
function p(
  ...
  const price = tomanToMoney(partial.priceToman)
  return {
    id: partial.id,
    slug: partial.slug,
    name: partial.name,
    ...
    variants: [
      {
        id: `variant-${partial.id}`,
        sku: `FIXTURE-${partial.id.toUpperCase()}`,
        title: null,
        price,
        attributes: [],
    ...
    price,
    oldPrice:
      partial.oldToman !== undefined ? tomanToMoney(partial.oldToman) : null,
...
function video(
  ...
  return {
    ...
    captions: [
      {
        url: '/media/demo/captions-fa.vtt',
        kind: 'captions',
        srclang: 'fa',
        label: 'فارسی',
      },
