...
function rowValues(row: ExcelJS.Row, headers: readonly string[]): string[] {
  return headers.map((_, index) => text(row.getCell(index + 1).value, false, `${row.worksheet.name}!${row.number}`) ?? '');
}

function parseSheet<T>(sheet: ExcelJS.Worksheet, headers: readonly string[], mapper: (values: string[]) => T): T[] {
...
export async function parseCatalogWorkbook(buffer: Buffer): Promise<ParsedCatalogWorkbook> {
  ...
  const products = parseSheet(workbook.getWorksheet('Products')!, HEADERS.Products, ([slug, name, description, brandSlug, categorySlug, status]) => ({ slug: text(slug, true, 'Products.slug')!, name: text(name, true, 'Products.name')!, description: text(description, false, 'Products.description'), brandSlug: text(brandSlug, false, 'Products.brandSlug'), categorySlug: text(categorySlug, false, 'Products.categorySlug'), status: text(status, false, 'Products.status') }));
  const variants = parseSheet(workbook.getWorksheet('Variants')!, HEADERS.Variants, ([productSlug, sku, barcode, title, costPrice, salePrice, weightGrams, status]) => ({ productSlug: text(productSlug, true, 'Variants.productSlug')!, sku: text(sku, true, 'Variants.sku')!, barcode: text(barcode, false, 'Variants.barcode'), title: text(title, false, 'Variants.title'), costPrice: text(costPrice, true, 'Variants.costPrice')!, salePrice: text(salePrice, true, 'Variants.salePrice')!, weightGrams: text(weightGrams, false, 'Variants.weightGrams'), status: text(status, false, 'Variants.status') }));
  const attributes = parseSheet(workbook.getWorksheet('Attributes')!, HEADERS.Attributes, ([code, name, description, status]) => ({ code: text(code, true, 'Attributes.code')!, name: text(name, true, 'Attributes.name')!, description: text(description, false, 'Attributes.description'), status: text(status, false, 'Attributes.status') }));
  const options = parseSheet(workbook.getWorksheet('AttributeOptions')!, HEADERS.AttributeOptions, ([attributeCode, code, label, status]) => ({ attributeCode: text(attributeCode, true, 'AttributeOptions.attributeCode')!, code: text(code, true, 'AttributeOptions.code')!, label: text(label, true, 'AttributeOptions.label')!, status: text(status, false, 'AttributeOptions.status') }));
