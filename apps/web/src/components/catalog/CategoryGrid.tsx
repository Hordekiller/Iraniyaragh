      ...
      <div className="bg-[#0F172A] rounded-[24px] lg:rounded-[28px] p-5 lg:p-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 lg:gap-4 mt-6">
          {categories.map(cat => {
            const Icon = cat.icon
            return (
              <button key={cat.id} onClick={() => navigate(ROUTES.category(cat.slug))} className="group relative overflow-hidden rounded-[20px] lg:rounded-[24px] bg-white p-4 lg:p-5 text-right hover:shadow-xl hover:shadow-black/40 transition-all duration-300 hover:-translate-y-1 border border-white">
