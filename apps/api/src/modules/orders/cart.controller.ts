...
export class CartController {
  ...
  @Put('lines/:variantId') set(@CurrentPrincipal() p: AuthPrincipalContext, @Headers('idempotency-key') key: string | undefined, @Param('variantId') variantId: string, @Body() body: CartLineDto): Promise<CartResponse> { if (!key?.trim() || !variantId.trim()) throw new BadRequestException({ code: 'INVALID_REQUEST', message: 'Idempotency-Key and variantId are required.' }); if (body.variantId !== variantId) throw new BadRequestException({ code: 'INVALID_REQUEST', message: 'variantId must match the request path.' }); return this.cart.setForUser(p.userId, variantId.trim(), body.quantity, key.trim()); }
