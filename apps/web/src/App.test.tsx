...
const routes: Array<[string, () => Promise<void> | void]> = [
  [
    '/',
    async () => {
      expect(await screen.findByLabelText('جستجو در محصولات')).toBeInTheDocument()
      expect(screen.getByRole('heading', { name: /تخصصی ابزار/ })).toBeInTheDocument()
    },
  ],
  [
  ...
  [
    ...
    async () => {
      expect(await screen.findByRole('heading', { name: /رونیکس ۲۲۱۰/ })).toBeInTheDocument()
    },
  ],
  [
    '/cart',
  ...
  [
    '/orders',
    async () => {
      expect(await screen.findByRole('heading', { name: /وارد شوید/ })).toBeInTheDocument()
    },
  ],
  [
    '/orders/IR-9999-1',
  ...
  [
    '/no-such-route',
    async () => {
      expect(await screen.findByRole('heading', { name: 'صفحه‌ای پیدا نشد' })).toBeInTheDocument()
    },
  ],
]
