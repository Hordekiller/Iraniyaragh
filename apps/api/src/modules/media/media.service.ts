...
import { PRODUCT_MEDIA_PROCESSING_QUEUE, type ProductMediaProcessingQueue } from "./processing-queue.port";

function sanitizedFilename(value: string): string {
  const cleaned = [...value.trim()]
    .map((character) => {
      const codePoint = character.codePointAt(0) ?? 0;
      return character === "/" || character === "\\" || codePoint <= 31 || codePoint === 127 ? "-" : character;
    })
    .slice(0, 255)
    .join("");
...
export class MediaService {
  constructor(
    ...
    @Inject(CatalogIdempotencyService)
  ...
  async initiateUpload(actorId: string, idempotencyKey: string, productId: string, input: ProductMediaUploadDto): Promise<ProductMediaUploadResponse> {
    ...
    const result = await this.idempotency.run<{ mediaId: string }>({
      ...
      execute: async (tx) => {
        ...
        const [activeCount, videoCount] = await Promise.all([
          tx.productMedia.count({
            where: { productId, state: { not: "ARCHIVED" } },
          }),
          tx.productMedia.count({
            where: { productId, kind: "VIDEO", state: { not: "ARCHIVED" } },
          }),
        ]);
        if (activeCount >= this.policy.maxActiveAssets || (input.kind === "VIDEO" && videoCount >= this.policy.maxActiveVideos)) {
          throw new UnprocessableEntityException({
  ...
  async confirmUpload(actorId: string, idempotencyKey: string, productId: string, mediaId: string, input: ProductMediaConfirmDto): Promise<ProductMediaConfirmResponse> {
    const current = await this.prisma.productMedia.findFirst({
    ...
    if (current.state === "PENDING_UPLOAD") {
      ...
      if (input.checksumSha256 && head.checksumSha256 && input.checksumSha256 !== head.checksumSha256) {
        throw new UnprocessableEntityException({
    ...
    const result = await this.idempotency.run<{ mediaId: string }>({
      ...
      execute: async (tx) => {
        const row = await tx.productMedia.findFirst({
          where: { id: mediaId, productId },
        });
        if (!row)
          throw new NotFoundException({
        ...
        if (row.state === "PENDING_UPLOAD") {
          ...
          if (updated.count !== 1) {
            throw new ConflictException({
              code: "STALE_VERSION",
              message: "Media version conflict.",
            });
          }
          await this.audit.record(
            {
    ...
    const confirmed = await this.prisma.productMedia.findUnique({
      where: { id: result.mediaId },
    });
    if (!confirmed)
      throw new NotFoundException({
  ...
  async updateMetadata(actorId: string, idempotencyKey: string, productId: string, mediaId: string, input: ProductMediaMetadataDto): Promise<ProductMediaConfirmResponse> {
    ...
    if ((altText && [...altText].length > 300) || (caption && [...caption].length > 500)) {
      throw new UnprocessableEntityException({
        ...
        message: "Media text is too long.",
      });
    }
    ...
    const updated = await this.prisma.productMedia.findUnique({
      where: { id: replay.mediaId },
    });
    if (!updated)
      throw new NotFoundException({
        code: "NOT_FOUND",
  ...
  async archive(actorId: string, idempotencyKey: string, productId: string, mediaId: string, input: ProductMediaArchiveDto): Promise<ProductMediaConfirmResponse> {
    const replay = await this.idempotency.run<{ mediaId: string }>({
      actorId,
        ...
        if (current.role === "PRIMARY") {
          ...
          if (product?.status === "ACTIVE") {
            throw new UnprocessableEntityException({
              ...
              message: "The primary image of a published product cannot be archived.",
            });
          }
        }
    ...
    const archived = await this.prisma.productMedia.findUnique({
      where: { id: replay.mediaId },
    });
    if (!archived)
      throw new NotFoundException({
  ...
  async reorder(actorId: string, idempotencyKey: string, productId: string, input: ProductMediaReorderDto): Promise<{ data: { items: AdminProductMedia[] } }> {
    const replay = await this.idempotency.run<{ mediaIds: string[] }>({
      ...
      execute: async (tx) => {
        ...
        if (input.items.length !== active.length || new Set(ids).size !== ids.length || ids.some((id) => !byId.has(id)) || positions.some((position, index) => position !== index) || input.items.some((item) => byId.get(item.mediaId)?.version !== item.expectedVersion)) {
          throw new ConflictException({
            code: "MEDIA_POSITION_CONFLICT",
            message: "Media ordering is stale or incomplete.",
          });
        }
        ...
        if (primary && input.items.find((item) => item.mediaId === primary.id)?.position !== 0) {
          throw new UnprocessableEntityException({
            code: "MEDIA_POSITION_CONFLICT",
            message: "Primary media must remain first.",
          });
        }
        ...
        for (const [index, item] of input.items.entries()) {
          ...
          if (moved.count !== 1) {
            throw new ConflictException({
              code: "MEDIA_POSITION_CONFLICT",
              message: "Media ordering changed concurrently.",
            });
          }
        }
    ...
    if (media.length !== replay.mediaIds.length) {
      throw new ConflictException({
        code: "MEDIA_POSITION_CONFLICT",
        message: "Media ordering changed after commit.",
      });
    }
    return { data: { items: media.map(adminMedia) } };
  }

  async setPrimary(actorId: string, idempotencyKey: string, productId: string, mediaId: string, input: ProductMediaPrimaryDto): Promise<{ data: { items: AdminProductMedia[] } }> {
    await this.idempotency.run({
      actorId,
      scope: `media.primary:${productId}`,
      key: idempotencyKey,
      payload: { productId, mediaId, ...input },
      execute: async (tx) => {
        const product = await tx.product.findUnique({
        ...
        const target = active.find((media) => media.id === mediaId);
        ...
        if (target.kind !== "IMAGE" || target.state !== "READY") {
          throw new UnprocessableEntityException({
            code: "MEDIA_NOT_READY",
            message: "Only a ready image can become the primary media.",
          });
        }
        ...
        const currentPrimary = active.find((media) => media.role === "PRIMARY");
        const displacedAtZero = active.find((media) => media.position === 0 && media.id !== target.id);
        if (displacedAtZero) {
          const released = await tx.productMedia.updateMany({
            where: {
              id: displacedAtZero.id,
              version: displacedAtZero.version,
              state: { not: "ARCHIVED" },
            },
            data: {
              ...(displacedAtZero.role === "PRIMARY" ? { role: "GALLERY" as const } : {}),
              position: 1_000_000,
              version: { increment: 1 },
            },
          });
        ...
        const promoted = await tx.productMedia.updateMany({
          where: {
            id: target.id,
            productId,
            version: input.expectedVersion,
          ...
          data: { role: "PRIMARY", position: 0, version: { increment: 1 } },
        });
        if (promoted.count !== 1)
          throw new ConflictException({
            code: "STALE_VERSION",
        ...
        await this.audit.record(
          {
            action: "catalog.media.primary.changed",
            entityType: "Product",
            entityId: productId,
            actorId,
            requestId: getRequestId(),
            before: { mediaId: currentPrimary?.id ?? null },
            after: { mediaId: target.id },
          },
          tx,
        );
        return {
  ...
  private assertRoleMatchesKind(kind: ProductMediaUploadDto["kind"], role: ProductMediaUploadDto["role"]): void {
