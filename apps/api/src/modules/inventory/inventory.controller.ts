...
export class InventoryController {
  ...
  async change(@CurrentPrincipal() principal: AuthPrincipalContext, @Headers('idempotency-key') idempotencyKey: string | undefined, @Headers('x-request-id') requestId: string | undefined, @Body() input: InventoryChangeDto) {
    return this.inventory.changeOnHand({ ...input, idempotencyKey, actorId: principal.userId, requestId: requestId ?? 'inventory-request' });
  }
