...
import type {
  ...
  ProductDetailResponse, ProductListResponse, ProductStatusResponse,
...
export class CatalogService {
  ...
  private async listProducts(query: Partial<ProductListQueryDto>, includeDrafts: boolean): Promise<ProductListResponse> {
    ...
    const status = includeDrafts ? statusToDb(query.status) : ProductStatus.ACTIVE;
  ...
  async getPublicProduct(idOrSlug: string): Promise<ProductDetailResponse> {
  ...
  async createBrand(actorId: string, input: BrandCreateDto): Promise<BrandResponse> {
    const brand = await this.prisma.brand.create({ data: { name: input.name.trim(), slug: input.slug }, include: { _count: { select: { products: true } } } });
  ...
  async updateBrand(actorId: string, id: string, input: BrandUpdateDto): Promise<BrandResponse> {
    const brand = await this.prisma.brand.update({ where: { id }, data: { ...(input.name ? { name: input.name.trim() } : {}), ...(input.slug ? { slug: input.slug } : {}) }, include: { _count: { select: { products: true } } } });
  ...
  async createCategory(actorId: string, input: CategoryCreateDto): Promise<CategoryResponse> {
    const category = await this.prisma.category.create({ data: { name: input.name.trim(), slug: input.slug, parentId: input.parentId }, include: this.categoryInclude() });
  ...
  async updateCategory(actorId: string, id: string, input: CategoryUpdateDto): Promise<CategoryResponse> {
    const category = await this.prisma.category.update({ where: { id }, data: { ...(input.name ? { name: input.name.trim() } : {}), ...(input.slug ? { slug: input.slug } : {}), ...(input.parentId !== undefined ? { parentId: input.parentId } : {}) }, include: this.categoryInclude() });
