...
export class CartService {
  ...
  async addForUser(
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (line && line.quantity + input.quantity > 99)
          throw new UnprocessableEntityException({
            code: 'CART_QUANTITY_INVALID',
            message: 'Quantity must be between 1 and 99.',
          });
        const nextQuantity = line
      ...
      if (
        ...
        const replay = await this.prisma.cartMutation.findUnique({
          where: {
            customerId_idempotencyKey: {
              customerId: customer.id,
              idempotencyKey: key,
        ...
        if (replay && replay.fingerprint === fingerprint)
          return replay.responseJson as unknown as CartResponse;
      }
      throw error;
    }
  }
  ...
  async removeForUser(
    ...
    const customer = await this.customer(userId);
    const fingerprint = createHash('sha256')
      .update(JSON.stringify({ variantId }))
      .digest('hex');
  ...
  async setForUser(
    ...
    const customer = await this.customer(userId);
    const fingerprint = createHash('sha256')
      .update(JSON.stringify({ variantId, quantity }))
      .digest('hex');
    try {
