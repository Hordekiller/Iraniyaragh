...
export function resolveMode(mode: AdminMode, systemPrefersDark: boolean): 'light' | 'dark' {
 ...
 * breaks the panel.
 */
export function parsePreferencesCookie(raw: string | undefined | null): AdminPreferences {
  ...
  try {
    ...
    return {
      mode: isOneOf(candidate.mode, MODES) ? candidate.mode : defaultPreferences.mode,
      skin: isOneOf(candidate.skin, SKINS) ? candidate.skin : defaultPreferences.skin,
      layout: isOneOf(candidate.layout, LAYOUTS) ? candidate.layout : defaultPreferences.layout,
