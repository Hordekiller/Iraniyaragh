...
|        2 | Issue #50 — Auth UX/E2E           | In progress        | separate fixture coverage from live integration; list remaining client states | Close only after accepted product evidence or split follow-up; the session/devices management slice is in open branch `feat/50-admin-session-management` (base `1de660e`) awaiting review |
