...
export function Bestsellers({ onSelectProduct }: BestsellersProps) {
  ...
  return (
    <div id={SECTION_IDS.bestseller} className="lg:col-span-8 bg-white rounded-[24px] lg:rounded-[28px] p-4 lg:p-6">
        ...
        {bestSellers.map(p => (
          <button key={p.id} type="button" onClick={() => onSelectProduct(p)} className="snap-start shrink-0 w-[160px] lg:w-[186px] rounded-[20px] border border-slate-100 overflow-hidden hover:shadow-lg hover:border-slate-200 transition cursor-pointer bg-slate-50 text-right">
            ...
            <div className="p-3">
              ...
              <div className="mt-2 flex items-baseline gap-1.5"><span className="font-black text-[14px] text-slate-900">{formatTomanDisplay(p.price)}</span><span className="text-[10px]">تومان</span></div>
      ...
      <button onClick={() => navigate(ROUTES.bestsellers)} className="w-full mt-4 h-11 rounded-full border-2 border-slate-900 text-slate-900 font-black text-sm hover:bg-slate-900 hover:text-white transition flex items-center justify-center gap-2">
