...
import { ImageIcon, Lock, PackagePlus } from "lucide-react";
...
import { catalogStatusLabel, catalogStatusTone, CATALOG_STATUS_META } from "@/lib/catalog/catalog-labels";
...
function normalizeQuery(raw: CatalogUrlQuery): CatalogProductsQuery {
  ...
  const status: CatalogProductsQuery["status"] = raw.status === "DRAFT" || raw.status === "PUBLISHED" || raw.status === "ARCHIVED" ? raw.status : undefined;
...
export function ProductsView({ initialQuery: raw }: { initialQuery: CatalogUrlQuery }) {
  ...
  const canReadMedia = canReadCatalogMedia(user);
  ...
  if (!canRead) {
    return (
      <>
        ...
        <EmptyState icon={<Lock size={28} />} title="دسترسی ندارید" description="حساب شما برای مشاهدهٔ کاتالوگ مجوز ندارد. برای دسترسی با مدیر سیستم هماهنگ کنید." />
      </>
    );
  }
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            {canWrite ? (
              <Button component={Link} href="/catalog/products/new" variant="contained" startIcon={<PackagePlus size={18} />}>
                کالای جدید
              </Button>
            ) : null}
          </>
      ...
      <DataTable<ProductListItem>
        ...
        columns={[
          ...
          },
          {
            id: "status",
            label: "وضعیت",
            render: (row) => <StatusChip label={catalogStatusLabel(row.status)} tone={catalogStatusTone(row.status)} />,
          },
          {
            id: "brand",
            label: "برند",
          ...
          },
          {
            id: "category",
            label: "دسته‌بندی",
            render: (row) => (row.categoryId && categoryById.has(row.categoryId) ? categoryById.get(row.categoryId) : "—"),
          },
          {
            id: "updatedAt",
        ...
        emptyDescription={query.search || query.status || query.brandId ? "با فیلترهای فعلی کالایی وجود ندارد؛ فیلترها را تغییر دهید یا از «کالای جدید» شروع کنید." : "هنوز کالایی ثبت نشده است. اولین کالا را با دکمهٔ «کالای جدید» بسازید."}
        search={query.search ?? ""}
        ...
        actions={(row) => (
          <Stack direction="row" spacing={0.5} alignItems="center">
            {canReadMedia ? (
              <Button component={Link} href={`/catalog/products/${row.id}/media`} size="small" startIcon={<ImageIcon size={16} />}>
                رسانه
              </Button>
            ) : null}
            {canWrite ? <ProductStatusActions product={row} onChanged={refresh} /> : null}
          </Stack>
        )}
        actionsLabel="عملیات"
        toolbar={
          <>
            <Select
              ...
              onChange={(event) =>
                apply({
                  status: (event.target.value || undefined) as CatalogProductsQuery["status"],
                })
              }
            >
            ...
            <Select size="small" value={query.brandId ?? ""} displayEmpty inputProps={{ "aria-label": "فیلتر برند" }} sx={{ minWidth: 150 }} onChange={(event) => apply({ brandId: (event.target.value as string) || undefined })}>
            ...
            <Select
              ...
              onChange={(event) =>
                apply({
                  categoryId: (event.target.value as string) || undefined,
                })
              }
            >
            ...
            {loading ? (
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                <Chip size="small" color="info" variant="outlined" label={`${new Intl.NumberFormat("fa-IR").format(meta.total)} کالا`} />
              </Stack>
            ) : null}
          </>
        }
