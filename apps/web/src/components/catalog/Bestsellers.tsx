    ...
    <div id="bestseller" className="lg:col-span-8 bg-white rounded-[24px] lg:rounded-[28px] p-4 lg:p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1.5">
          <button onClick={() => scrollCarousel(bestRef, 'right')} className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-900 hover:text-white transition"><ChevronRight size={16} /></button>
          <button onClick={() => scrollCarousel(bestRef, 'left')} className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-900 hover:text-white transition"><ChevronLeft size={16} /></button>
        </div>
      </div>

      <div ref={bestRef} className="flex gap-3 overflow-x-auto scrollbar-none snap-x snap-mandatory mt-5 pb-2 -mx-1 px-1">
        {bestSellers.map(p => (
          <div key={p.id} onClick={() => onSelectProduct(p)} className="snap-start shrink-0 w-[160px] lg:w-[186px] rounded-[20px] border border-slate-100 overflow-hidden hover:shadow-lg hover:border-slate-200 transition cursor-pointer bg-slate-50">
