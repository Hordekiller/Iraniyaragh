...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Platform/current | Order expiry/cancellation compensation implemented on `feat/epic5-order-lifecycle` (open PR, all API gates green); #114 remains externally gated | API, data, contracts and operations only; new worktree from current `main`         | #246/#247 are the accepted Checkout/Order baseline; the command slice in the Epic-5 branch changed `schema.prisma` (new `OrderCommandIdempotencyRecord` table + `StockReservation` index) via its own forward migration — rebase any migration touching these files |
