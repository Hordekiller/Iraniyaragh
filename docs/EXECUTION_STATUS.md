...
| `0.3` Inventory       | Foundation + protected HTTP        | Transactional ledger/reservation service; protected balance/adjustment API merged (#217) with declared error-code wiring and request-ID binding (#219); #215 HTTP contract is under review | Warehouse/location CRUD, HTTP reservations, transfers, expiry worker and admin (EXEC_G4 / #215) |
