...
- Unit/HTTP tests cover environment/CORS validation, database URL safety and
  ...
  verification) plus Auth persistence constraints and Session rotation/replay
  concurrency (exactly one concurrent refresh winner).
- Auth storage and lifecycle constraints now include the runtime Session/MFA evidence,
  and the Session core rotates/revokes refresh families transactionally. The
  migration-independent token/hash primitives exist; ADR-0007 and `AUTH_CONTRACT.md`
  define the remaining runtime, HTTP, threat and client-state contract. HTTP Auth
  endpoints, credential verification, transactional refresh at the API surface, OTP
  delivery, rate limiting and password/TOTP verification remain unimplemented.
